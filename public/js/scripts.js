...
(function($){
 $(function() {
  var LPITCH = {
   block_heights : function(ele, extra) {
    ele.css('height', 'auto');        
                ...
                ele.each(function() {
                    ...
                    if (currentRowStart != topPostion) {
                     if (rowDivs.length != 1) {                     
                         ...
                         currentTallest = $el.height() + extra;
                         rowDivs.push($el);
                        }
                    } else {
                    ...
                    // do the last row
                    for (currentDiv = 0; currentDiv < rowDivs.length; currentDiv++) {
                        rowDivs[currentDiv].height(!currentTallest ? 'auto' : currentTallest);
                    }
                 }); 
   } ,
   template_resize_blocks : function(ele, extra) {    
    setTimeout(function() {     
     if (ele.find('img').length == ele.find('img.loaded').length) {
      LPITCH.block_heights(ele, extra);
     }
     else {
      LPITCH.template_resize_blocks(ele, extra);
     }
    }, 100);
   } ,   
   template_adjustments : function() {
    var ele = $('.field-name-field-items .nodewrap__fields');
    ...
    var extra = parseInt(ele.css('margin-bottom').replace('px','')) +
